...
public class CibilScoreSearchControllerTest {
    ...
    @Test
    public void createCibilScoreAPI() throws Exception {

        when(cibilScoreService.saveCibilScore(Mockito.any())).thenReturn(new CibilScoreDetails("firstName4", 800.0,"BGHDSS877233"));
        mvc.perform(MockMvcRequestBuilders.post("/api/v1/creditcard")
                .content(asJsonString(new CibilScoreDetails("firstName4", 800.0,"BGHDSS877233"))).contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.name").exists())
                .andExpect(MockMvcResultMatchers.jsonPath("$.cibilScore").exists())
                .andExpect(MockMvcResultMatchers.jsonPath("$.panCard").exists());

        verify(cibilScoreService).saveCibilScore(Mockito.any());
    }
